...
void Signal_PWM_Captured(int32_t thr)
{
	static uint8_t startupStatus = 0;
	lastCapMS = millis();
	int interval = LTBL_GetAvgCommInterval();
	...
	if(thr > 0)
	{
		...
		if(interval > LowRPMProtect_StartIntv)
		{
			...
			if(interval > LowRPMProtect_Startup1Intv1)
			{
				startupStatus = 1;
			}
			else if(interval < LowRPMProtect_Startup1Intv2)
			{
				startupStatus = 0;
			}
			if(LTBL_Default_PWM_Mode == LTBL_PWM_MODE_SINGLE && 
				CONFIG_UseStartup1 == YES && startupStatus)
			{
					LTBL_SetMode(LTBL_Mode_Startup1);
			}
			else
			{
